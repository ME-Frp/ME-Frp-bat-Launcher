...
ECHO. 6.访问官网
ECHO.
ECHO.  ? 2022-2023 镜缘映射运行组和开发者(某贼)保留一切权利.
set choice=1
set /p choice= 输入数字按Enter继续：
if "%choice%"=="1" goto STARTFRP
if "%choice%"=="2" goto EDITINI
if "%choice%"=="3" goto RESETLAUNCHER
::if "%choice%"=="4" start https://www.mefrp.com/ & echo.逆向32038正向32032>tmp.txt & clip < tmp.txt & del tmp.txt
...
:editini-9
curl "https://api.mefrp.com/api/v1/?action=getconf&&apitoken=MEFRPToken|%nodenum%&token=%userkey%&node=%nodenum%" 1>>配置文件\tmp2.txt 2>nul
