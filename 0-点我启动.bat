...
COLOR 0E
TITLE ME FRP 启动器 beta 20220419_2
...
::if "%choice%"=="4" start https://frp.msrx.online/?page=panel&module=addproxy & echo.逆向32038正向32032>tmp.txt & clip < tmp.txt & del tmp.txt
if "%choice%"=="4" goto USERKEY
if "%choice%"=="5" goto INSTRUCTION
if "%choice%"=="6" start https://frp.msrx.online/
goto START
...
ECHO.
ECHO.
ECHO. 首先你需要访问 ME FRP 官网，注册一个账号。
ECHO. https://frp.msrx.online
ECHO.
...
ECHO.
ECHO.
ECHO.1.自动导入(默认)   2.手动导入   3.从旧版本导入
ECHO.
